		...
		<tbody>
			...
			foreach ( $customer_orders->orders as $customer_order ) {
				...
				<tr
					...
					data-order-row
					data-order-url="<?php echo esc_url( $view_order_url ); ?>"
					tabindex="0"
					role="link"
					aria-label="<?php echo esc_attr( sprintf( __( 'Open order number %s', 'woocommerce' ), $order->get_order_number() ) ); ?>"
				>
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) :
					...
					?>
						...
						<?php endif; ?>
							...
							<?php elseif ( $is_order_number ) : ?>
								<a href="<?php echo esc_url( $view_order_url ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'View order number %s', 'woocommerce' ), $order->get_order_number() ) ); ?>">
							...
							<?php elseif ( 'order-status' === $column_id ) : ?>
								<?php
								$status_slug = sanitize_title( (string) $order->get_status() );
								?>
								<span class="velune-order-status-badge velune-order-status-badge--<?php echo esc_attr( $status_slug ); ?>"><?php echo esc_html( wc_get_order_status_name( $order->get_status() ) ); ?></span>
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
									echo '<div class="velune-order-actions-wrap">';
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										...
										$action_tone_class = 'pay' === $key ? 'velune-order-action--primary' : 'velune-order-action--secondary';

										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button' . esc_attr( $wp_button_class ) . ' button ' . sanitize_html_class( $key ) . ' velune-order-action ' . esc_attr( $action_tone_class ) . '" aria-label="' . esc_attr( $action_aria_label ) . '">' . esc_html( $action['name'] ) . '</a>';
